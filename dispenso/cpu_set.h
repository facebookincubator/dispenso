...
namespace detail {
#if defined(DISPENSO_CPUSET_LINUXY)
// The native affinity-set type: cpu_set_t on Linux, cpuset_t on FreeBSD. Aliased
// in detail rather than as a global `typedef cpuset_t cpu_set_t` so that
// including this header does not inject a cpu_set_t name into every translation
// unit (which could clash with a consumer's own definition).
#if defined(__FreeBSD__)
using CpuSetNative = ::cpuset_t;
#else
using CpuSetNative = ::cpu_set_t;
#endif
#endif // DISPENSO_CPUSET_LINUXY
} // namespace detail
...
struct CacheGroup {
  ...
  int32_t cacheId; ///< Unique cache identifier. On Linux, the hardware cache
                   ///< instance ID from sysfs; on FreeBSD, a dispenso-assigned
                   ///< counter with no hardware/OS meaning.
};
